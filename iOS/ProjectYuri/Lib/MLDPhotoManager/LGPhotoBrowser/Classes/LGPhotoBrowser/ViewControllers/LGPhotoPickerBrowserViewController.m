...
- (void)selectedButtonTapped {
    ...
    if (self.selectedAssets.count >= maxCount && !_imagePickerSelectView.selectBtn.selected) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:format delegate:self cancelButtonTitle:@"好的" otherButtonTitles:nil];
        [alert show];
        return ;
    }
...
- (void)pickerPhotoScrollViewDidSingleClick:(LGPhotoPickerBrowserPhotoScrollView *)photoScrollView{
    ...
    self.imagePickerSelectView.hidden = alphaValue;
    self.XGtoolbar.hidden = alphaValue;
}
